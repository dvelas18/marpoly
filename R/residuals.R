mp_lag_coefficients <- function(causal = NULL, noncausal = NULL) {
  ...
  if (r > 0L) for (i in seq_len(r)) vals[as.character(i)] <- vals[as.character(i)] - causal[i]
  if (s > 0L) for (j in seq_len(s)) vals[as.character(-j)] <- vals[as.character(-j)] - noncausal[j]
  if (r > 0L && s > 0L) {
    for (i in seq_len(r)) {
      for (j in seq_len(s)) {
        lag <- i - j
...
marpoly_residuals <- function(x, causal = NULL, noncausal = NULL, drop = TRUE,
  ...
  causal <- mp_as_vector(causal, "causal")
  noncausal <- mp_as_vector(noncausal, "noncausal")
  if (exists("MAR_model_errors", mode = "function")) {
    e <- MAR_model_errors(x, phi_plus = causal, phi_star = noncausal, demean = FALSE)
    if (!drop) {
      out <- rep(NA_real_, length(x))
      idx <- as.integer(names(e))
      out[idx] <- e
      return(out)
    }
    attr(e, "time_index") <- as.integer(names(e))
    return(e)
  }
  n <- length(x)
  r <- length(causal)
  ...
  if (start > end) stop("Not enough observations to compute residuals for this MAR order.", call. = FALSE)
  for (t in start:end) {
    val <- 0
    for (j in seq_len(nrow(lag_table))) val <- val + lag_table$coefficient[j] * x[t - lag_table$lag[j]]
    eps[t] <- val
  }
  if (drop) {
    out <- eps[start:end]
...
mp_errors_from_root_table_row <- function(x, row) {
  r <- as.integer(row$r[1L])
  s <- as.integer(row$s[1L])
  pp <- if (r > 0L) as.numeric(row[1L, paste0("phi_plus_", seq_len(r))]) else numeric(0)
  ps <- if (s > 0L) as.numeric(row[1L, paste0("phi_star_", seq_len(s))]) else numeric(0)
  MAR_model_errors(x, phi_plus = pp, phi_star = ps, demean = FALSE)
}

#' Extract residuals/errors from a fitted MAR allocation
#'
#' @param object A `marpoly_fit` object.
#' @param allocation "selected", "all", or an allocation rank/id.
...
#' @param allocation "selected", "all", or an allocation rank/id.
#' @param demean If TRUE, demean before filtering when using legacy fit objects.
...
marpoly_errors <- function(fit, allocation = "selected", demean = TRUE) {
  ...
  if (inherits(fit, "MAR_selection")) {
    if (is.null(fit$x)) stop("The fitted object does not contain the original series.", call. = FALSE)
    if (identical(allocation, "selected")) {
      sf <- mp_selected_fit(fit)
      if (!is.null(sf$errors)) return(sf$errors)
      return(MAR_model_errors(fit$x, phi_plus = sf$phi_plus, phi_star = sf$phi_star, demean = FALSE))
    }
    tab <- fit$all_root_allocations
    if (identical(allocation, "all")) {
      out <- vector("list", nrow(tab))
      for (i in seq_len(nrow(tab))) out[[i]] <- mp_errors_from_root_table_row(fit$x, tab[i, , drop = FALSE])
      names(out) <- paste0(seq_len(nrow(tab)), "_", tab$model, "_root", tab$root_combination)
      return(out)
    }
    id <- as.integer(allocation)
    if (!is.finite(id) || id < 1L || id > nrow(tab)) stop("allocation must be 'selected', 'all', or a valid ranked row number.", call. = FALSE)
    return(mp_errors_from_root_table_row(fit$x, tab[id, , drop = FALSE]))
  }
  if (identical(allocation, "selected")) {
